...
def get_activations(images, sess, batch_size=128, verbose=True):
    ...
    pbar = tqdm.tqdm(np.arange(n_batches))
    for i in pbar:
        if verbose:
            progress_str = "\rPropagating batch %d/%d" % (i+1, n_batches)
...
def calculate_activation_statistics(images, sess, batch_size=128, verbose=True):
...
def get_activations_from_files(files, sess, batch_size=128, verbose=True):
    """Calculates the activations of the pool_3 layer for all images.
    ...
    n_imgs = (n_imgs//batch_size)*batch_size
    ...
    for i in pbar:
        ...
        if verbose:
            progress_str = "\rPropagating batch %d/%d, batch.shape: %s" % (i+1, n_batches, str(batch.shape))
            pbar.set_description(progress_str)
...
def calculate_activation_statistics_from_files(files, sess, batch_size=128, verbose=True):
    """Calculation of the statistics used by the FID.
    Params:
    -- files      : list of paths to image files. Images need to have same dimensions for all files.
    -- sess        : current session
...
def _handle_path(path, sess, low_profile=False, save_stats=True, stats_path=None):
    if path.endswith('.npz'):
        ...
        if(save_stats):
            if(stats_path is None):
                stats_path = os.path.join(path, 'fid_stats.npz')
            np.savez_compressed(stats_path, mu=m, sigma=s)
    return m, s
