...
def get_activations(images, sess, batch_size=50, verbose=False):
    ...
    pbar = tqdm.tqdm(np.arange(n_batches), leave=False)
    for i in pbar:
        if verbose:
...
def calculate_frechet_distance_jax(mu1, sigma1, mu2, sigma2, eps=1e-6):
    """Numpy implementation of the Frechet Distance.
    The Frechet distance between two multivariate Gaussians X_1 ~ N(mu_1, C_1)
    and X_2 ~ N(mu_2, C_2) is
            d^2 = ||mu_1 - mu_2||^2 + Tr(C_1 + C_2 - 2*sqrt(C_1*C_2)).
    ...
    covmean = jnp.linalg.cholesky(prod)
...
def _handle_path(path, sess, low_profile=False, save_stats=True):
    if path.endswith('.npz'):
        ...
        if(save_stats):
            output_path = os.path.join(path, 'fid_stats.npz')
            np.savez_compressed(output_path, mu=m, sigma=s)
    return m, s
